...
const domLoaded = () => {
  ...
  loadBootstrapTimePicker();
  enableBootstrapTimePicker();
};
...
const enableBootstrapTimePicker = () => {
  $(".datetimepicker").each((index, elem) => {
    ...
    if ($elem.data("format")) option.format = $elem.data("format");
    if ($elem.data("use-current"))
      option.useCurrent = $elem.data("use-current");
    if ($elem.data("show-today-button"))
      option.showTodayButton = $elem.data("show-today-button");
    if ($elem.data("inline")) option.inline = $elem.data("inline");
    if ($elem.data("locale")) option.locale = $elem.data("locale");
    if ($elem.data("day-view-header-format"))
      option.dayViewHeaderFormat = $elem.data("day-view-header-format");
 ...
 *
 * Embed core source code since js module loading problem
 * it is not easy to combine parcel bundler and moment locale js
 */
const loadBootstrapTimePicker = () => {
